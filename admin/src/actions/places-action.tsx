...
export const addPlace = async (
  data: AddPlacePayload,
  dataPlaceImage: File[],
  dataDescriptionImage: File[]
): Promise<void> => {
  const token = await utils.getAuthTokenFromServerCookies();
  ...
  try {
    if (dataPlaceImage.length > 0) {
      const uploadResult = await cloudinaryAction.UploadImage(
        dataPlaceImage,
        data.name + "/images"
      );
      ...
      for (let i = 0; i < place.placeImages.length; i++) {
        try {
          const response = await client<void>(
            `/cloudinary?publicId=${place.placeImages[i].imagePublicId}`,
            {
              method: "DELETE",
              headers: {
                "Content-Type": "application/json",
